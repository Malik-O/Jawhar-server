...
exports.summarizeWithGroq = summarizeWithGroq;
exports.fixTranscript = fixTranscript;
exports.fixTranscriptWithSpeakers = fixTranscriptWithSpeakers;
...
const buildUserPrompt = (transcript) => `فيما يلي نص محاضرة مفرّغ من ملف صوتي. قم بمعالجته وأنتج:
...
function isRateLimitError(error) {
    if (error instanceof groq_sdk_1.default.RateLimitError)
        return true;
...
async function summarizeWithGroq(transcript) {
    ...
    throw new Error(`فشل التلخيص: تم استنفاد حصة جميع النماذج المجانية المتاحة.\n` +
        `تفاصيل: ${errors.join(' | ')}\n` +
 ...
 */
async function fixTranscriptWithSpeakers(segments) {
...
//# sourceMappingURL=geminiProcessor.js.map